...
class NotificationScreen extends StatefulWidget {
  ...
  final String? token;
  NotificationScreen({Key? key, required this.messages, required this.token})
      : super(key: key);
  @override
...
class _NotificationScreenState extends State<NotificationScreen> {
  ...
  String constructFCMPayload(String? token) {
    return jsonEncode({
      'notification': <String, dynamic>{'body': 'Text', 'title': 'Text title'},
      'priority': 'high',
      'data': <String, dynamic>{
              ...
              children: [
                ListView.builder(
                    ...
                    itemBuilder: (context, index) {
                      ...
                      var sendTime = DateTime.parse(message.sentTime.toString())
                              ...
                              Expanded(
                                child: Text(
                                  message.notification?.title ??
                                      'no RemoteMessage. available',
                                  style: TextStyle(fontWeight: FontWeight.bold),
                                ),
                              ),
                              Text((sendTime.isNegative
                                      ? (sendTime * -1).toString()
                                      : (sendTime.toString())) +
                                  " gio truoc")
                            ],
